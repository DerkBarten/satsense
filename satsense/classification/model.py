...
def create_train_test():
    slum_mask_path = "{base_path}/{image_name}_masked_slum.tif".format(base_path=base_data_path, image_name=image_name)
    building_mask_path = "{base_path}/{image_name}_masked_building.tif".format(base_path=base_data_path, image_name=image_name)
        ...
        generator = CellGenerator(image=sat_image, size=main_window_size)
        feature_vector = extract_features(feature_set, generator, load_cached=cached, image_name=image_name)

        X_0, y_0 = Dataset(feature_vector).filter(building_mask, label=0)
        X_1, y_1 = Dataset(feature_vector).filter(slum_mask, label=1)
        
        X_train = ..
        y_train = ..

        X_test = Dataset(feature_vector).all()
